...
class AdvancedPasswordManagement
{
	...
	protected function isBruteForce($userID)
	{
		...
		$result = SQL('SELECT `PWD_TOTAL_LOGIN_ATTEMPTS`, `PWD_LAST_LOGIN_ATTEMPT`, `PWD_FIRST_LOGIN_ATTEMPT` FROM `'._UFO_RBAC_SCHEMA_.'`.`'.$db->manageCase('T_PASSWORD_PWD').'` WHERE USR_ID = ?', array($userID));
		
		//if first_login_attempt OR last_login_attempt are not set, then set them and return false.
		if ( ($result[0]['PWD_FIRST_LOGIN_ATTEMPT'] == 0) || ($result[0]['PWD_LAST_LOGIN_ATTEMPT'] == 0) )
		{
			SQL('UPDATE `'._UFO_RBAC_SCHEMA_.'`.`'.$db->manageCase('T_PASSWORD_PWD').'` SET `PWD_TOTAL_LOGIN_ATTEMPTS` = `PWD_TOTAL_LOGIN_ATTEMPTS` + 1, `PWD_FIRST_LOGIN_ATTEMPT` = ?, `PWD_LAST_LOGIN_ATTEMPT` = ? WHERE USR_ID = ?', array($currentTime, $currentTime, $userID));
			return FALSE;
		}
		...
		if ( ($currentTime - $result[0]['PWD_LAST_LOGIN_ATTEMPT']) <= AdvancedPasswordManagement::$bruteForceLockTimePeriod )
		{
			SQL('UPDATE `'._UFO_RBAC_SCHEMA_.'`.`'.$db->manageCase('T_PASSWORD_PWD').'` SET `PWD_TOTAL_LOGIN_ATTEMPTS` = ?, `PWD_FIRST_LOGIN_ATTEMPT` = ?, `PWD_LAST_LOGIN_ATTEMPT` = ? WHERE USR_ID = ?', array(0, 0, 0, $userID));
			return TRUE;
		}
		
		//check if two subsequent requests are made within $bruteForceLockAttemptTotalTime time-period.
		if ( ($currentTime - $result[0]['PWD_FIRST_LOGIN_ATTEMPT']) <= AdvancedPasswordManagement::$bruteForceLockAttemptTotalTime )
		{
			...
			if ($result[0]['PWD_TOTAL_LOGIN_ATTEMPTS'] >= AdvancedPasswordManagement::$bruteForceLockAttempts)
			{
				SQL('UPDATE `'._UFO_RBAC_SCHEMA_.'`.`'.$db->manageCase('T_PASSWORD_PWD').'` SET `PWD_TOTAL_LOGIN_ATTEMPTS` = ?, `PWD_FIRST_LOGIN_ATTEMPT` = ?, `PWD_LAST_LOGIN_ATTEMPT` = ? WHERE USR_ID = ?', array(0, 0, 0, $userID));
				return TRUE;
			}
			...
			{
				SQL('UPDATE `'._UFO_RBAC_SCHEMA_.'`.`'.$db->manageCase('T_PASSWORD_PWD').'` SET `PWD_TOTAL_LOGIN_ATTEMPTS` = `PWD_TOTAL_LOGIN_ATTEMPTS` + 1, `PWD_LAST_LOGIN_ATTEMPT` = ? WHERE USR_ID = ?', array($currentTime, $userID));
				return FALSE;
			}
		}
	...
	public static function checkIfTempPassExpired($userID)
	{
		...
		if (count($result) == 1)
		{
			if ( (time() - $result[0]['PWD_TEMP_TIME'])  < AdvancedPasswordManagement::$tempPassExpiryTime)
				return FALSE;
		}
	...
	public static function tempPassword($userID, $tempPass = "")
	{
		...
		if ($tempPass == "")
		{
			$tempPass = hash(BasicPasswordManagement::$hashAlgo, \Ufo\Core\Init\randstr(128));
		...
		else	//If a temp pass is provided, then check if it is not expired and it correct.
		{
			...
			if (count($result) == 1)
			{
				...
				if ( ($result[0]['PWD_USE_FLAG'] == 0) && (! $a = AdvancedPasswordManagement::checkIfTempPassExpired($userID)) )
				{	
