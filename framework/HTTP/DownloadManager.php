 ...
 * Child Exceptions
 */
class InvalidFileModifiedDateException extends Exception\DownloadManagerException {}
			...
			while (ftell($f) <= $seek_end)
			{
				...
				if ( ($advanceBy + $currentPos) > $seek_end )	//check if you have reached the position where you have to stop reading.
					$advanceBy = $seek_end - $currentPos;
                                    ...
                                    \Ufo\Core\Init\echof(fread($f, $advanceBy));	//read the specified number of bytes.
