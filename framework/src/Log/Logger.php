 ...
 * Child Exceptions
 */
class MediaNotSupported extends Exception\LogException {}
class UnsupportedConfigFile extends Exception\LogException {}
...
class Logger
{
	 ...
	 */
	public function __construct( $pathToConfigFile = "{_UFO_PROFILES_LOG_DIR_}{_UFO_DEFAULT_LOG_PROFILE_}")
	{
		...
		if ( file_exists( __DIR__ . "/media/" . $storageType . ".php" ) )	//IF that storage medium is supported, then
		{
			...
			$storageType = "\Ufo\Log\\" . $storageType;	
